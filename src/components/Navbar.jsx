  ...
  return (
    ...
    >
      <div className="max-w-screen-xl mx-auto flex justify-between items-center px-6 md:px-12 w-full">
        ...
        {/* Conteneur logo + LanguageSwitcher */}
        <div className="flex items-center space-x-4">
          ...
          {/* LanguageSwitcher ici, inline, pas en fixed */}
          <div>
            <LanguageSwitcher />
          </div>
        </div>
        ...
        <ul className="hidden md:flex space-x-12 font-semibold tracking-wide text-gray-300">
          {links.map((link) => (
            <li key={link} className="relative group cursor-pointer">
              ...
              <span className="absolute left-0 bottom-0 w-0 h-0.5 bg-cyan-400 group-hover:w-full transition-[width] duration-300 ease-in-out"></span>
            </li>
          ))}
        </ul>
        ...
        >
          <span
            className={`block w-8 h-0.5 bg-cyan-400 rounded-sm transform transition duration-300 origin-left ${
              navOpen ? "rotate-45 translate-y-2" : ""
            }`}
          ></span>
          <span
            className={`block w-8 h-0.5 bg-cyan-400 rounded-sm transition duration-300 ${
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed top-20 left-0 right-0 bg-black bg-opacity-90 backdrop-blur-md md:hidden overflow-hidden transition-[max-height] duration-500 ease-in-out ${
